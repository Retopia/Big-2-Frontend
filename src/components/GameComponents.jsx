...
function Card({ suit, value, faceUp = true, isSelected = false }) {
  ...
  const textColor = (suit === '♥' || suit === '♦') ? 'text-red-600' : 'text-gray-900';
  ...
  return (
    <div 
      ...
      ${isSelected ? 'shadow-lg ring-2 ring-yellow-400' : 'shadow'} 
      select-none relative`}
    >
      {faceUp ? (
        <div className={`w-full h-full flex flex-col justify-between p-1 ${textColor}`}>
          <div className="text-sm font-bold leading-none">{displayValue}</div>
          <div className="text-4xl absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">{suit}</div>
          <div className="text-sm font-bold self-end leading-none">{displayValue}</div>
        </div>
      ) : (
...
function OpponentHand({ playerName, cardCount, isActive }) {
  return (
    <div className={`mb-2 p-3 rounded-lg ${isActive ? 'bg-gray-700' : 'bg-gray-800'}`}>
      <div className="flex items-center">
        ...
        <div className="flex items-center">
          ...
          <h3 className={`text-md font-medium ${isActive ? 'text-yellow-400' : 'text-white'}`}>
            {playerName}{isActive && <span className="text-sm ml-1.5 text-yellow-300">(Active)</span>}
          </h3>
        </div>
    ...
    <div className="my-4 p-4 bg-gray-800 rounded-lg">
      <div className="mb-2 flex justify-between items-center">
        <h2 className="text-lg font-bold">
          Turn: <span className={isYourTurn ? "text-yellow-400" : "text-blue-400"}>{currentTurn}</span>
          {isYourTurn && <span className="ml-2 text-yellow-400 animate-pulse">(Your Turn)</span>}
        </h2>
      </div>
...
function GameDisplay({ gameState, socket }) {
  ...
  const getSuitValue = (card) => {
    ...
    let suitValue;
    switch (card.suit) {
      case '♣': suitValue = 1; break;
      case '♦': suitValue = 2; break;
      case '♥': suitValue = 3; break;
      case '♠': suitValue = 4; break;
      default: suitValue = 0;
    }
  ...
  const sortedHand = [...(gameState.hand || [])].sort((a, b) => 
    sortBySuit ? getSuitValue(a) - getSuitValue(b) : getCardValue(a) - getCardValue(b)
  );
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex space-x-3">
          <div className="flex items-center bg-gray-800 rounded-full px-3 py-1.5">
            ...
            <div className="relative flex items-center justify-between bg-gray-700 w-20 h-7 rounded-full p-1 cursor-pointer" onClick={() => setSortBySuit(!sortBySuit)}>
              <span className={`inline-block text-xs z-10 pl-1.5 ${sortBySuit ? 'text-gray-400' : 'text-white font-medium'}`}>Value</span>
              <span className={`inline-block text-xs z-10 pr-1.5 ${sortBySuit ? 'text-white font-medium' : 'text-gray-400'}`}>Suit</span>
      ...
      <div className="mt-6">
        ...
        {isMyTurn && (
          <p className="text-center mt-2 text-yellow-400 animate-pulse text-sm">
